...
Suppose there are 2 populations, X and Y. The population means $\mu_1$ and $\mu_2$ are both 5, and the population variance are both 4 (i.e., population standard deviation $\sigma$ is 2). Since in reality often we don't know population means, as a result we'd like to test if population means are truly the same.

For each population, we draw samples with size of 5^[The reason we choose such small sample sizes is so that the difference between distributions of statistics are more obvious.], and we do this 1e5 times. This way we can get the sample mean and sample standard deviantion for all 1e5 draws.

```{r lesson1_example2_pt1}
s.ids = as.list(1:1e5)

GenerateSample = function(id, s.size = 30, pop.mean = 5, pop.sd = 2){
  sampling = rnorm(s.size, pop.mean, pop.sd)
  return(data.frame(sample_id = id,
                    value = sampling))
}

df.samples.1 = ldply(s.ids, function(x) GenerateSample(x, s.size = 5))
df.samples.2 = ldply(s.ids, function(x) GenerateSample(x, s.size = 5))
...
3. Calculate the statistics' values.
...
However, more often than not we don't know what are the values of population variances. In that case, we construct below statistic, where sample variances $S_x^2$ and $S_y^2$ are used to estimate $Var_1$ and $Var_2$: 
  
$$
stat_2 = \dfrac {(\bar X - \bar Y) - (\mu_1 - \mu_2)}{\sqrt{\dfrac{S_x^2}{n_1} + \dfrac{S_y^2}{n_2}}}
$$

The distribution of $stat_1$ is clearly z-distribution (i.e., standard normal distribution). What about $stat_2$? To find out, we plot out the distributions of $stat_1$ and $stat_2$ to see what they look like.

```{r lesson1_example2_pt2}
# Construct the first test statistic.
test.stat.1 = 
  (df.s.stats.1$sample_mean - df.s.stats.2$sample_mean) /
  (sqrt(2^2 / 5 + 2^2 / 5))

# Construct the secondo test statistic.
test.stat.2 = 
  (df.s.stats.1$sample_mean - df.s.stats.2$sample_mean) /
  (sqrt(df.s.stats.1$sample_sd^2 / 5 + df.s.stats.2$sample_sd^2 / 5))
...
df.final = bind_rows(
  ...
  tibble(test = "standard normal", statistic_value = rnorm(1e5)),
  tibble(test = "student's t", statistic_value = rt(1e5, 5 + 5 -2))
)
...
grid.arrange(p.l1.1, p.l1.2, ncol = 2)
```

We can see that the 2 distributions differ slightly. Next we compare the $stat_1$ with a standard normal distribution and $stat_2$ with a t-distribution.

```{r lesson1_example2_pt3}
# Compare statistic with standard normal distribution
p.l1.3 = ggplot(df.final %>% 
                  filter(test %in% c("stat_1", "standard normal"))) +
  geom_density(aes(statistic_value, color = test, fill = test), alpha = 0.2)
...
p.l1.4 = ggplot(df.final %>% 
                  filter(test %in% c("stat_2", "student's t"))) +
  geom_density(aes(statistic_value, color = test, fill = test), alpha = 0.2)

p.l1.4
```

As we can see. The $stat_1$ statistic follows a standardized normal distribution, and $stat_2$ statistic follows a student t's distribution with a degree of freedom of 8 (How is degree of freedom determined? We'll discuss that shortly). 

If sample size were larger (e.g., 30 instead of 5), then both $stat_1$ and $stat_2$ would follow standard normal distribution.
...
#### What is the degree of freedome of the t test statistic?
